...
No codigo lido, essa camada aparece na fachada packages/ag-ui-runtime/index.js e nos modulos compartilhados dentro de app/ui/static/js/shared. E ela que recebe o stream vindo de /ag-ui/runs, reconstrui o estado e renderiza a experiencia.
...
2. createAgUiSseClient executa fetch POST para /ag-ui/runs com Accept text/event-stream;
3. o cliente extrai X-Correlation-Id dos headers e repassa ao consumidor;
4. o parser SSE reconstrui os blocos event e data incrementalmente;
...
O cliente usa fetch e le a resposta SSE manualmente. Isso permite enviar payload completo de run no corpo do request.
...
- paginas que reimplementem parsing SSE de forma diferente podem divergir do comportamento canônico;
...
- erro de parse SSE ou falha HTTP reportada pelo client;
...
- app/ui/static/js/shared/ag-ui-client.js
  ...
  - Comportamento confirmado: consumo de POST SSE com captura de X-Correlation-Id e reconexao opcional.
...
- app/ui/static/js/shared/ag-ui-sidecar-chat.js
  ...
  - Comportamento confirmado: adaptacao de interrupcao AG-UI ao painel HIL compartilhado e montagem do resume oficial.
