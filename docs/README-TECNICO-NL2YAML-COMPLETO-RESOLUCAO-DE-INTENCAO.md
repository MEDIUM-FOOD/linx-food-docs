...
O contrato público aceita quatro targets:
...
Quando o target já vem explícito, ele prevalece como contrato da chamada. Quando vem como auto, o parser híbrido usa pesos por sinais textuais para escolher entre workflow, supervisor clássico e deepagent.
...
Se o texto não deixa claro se o caso é workflow, supervisor ou deepagent, o parser não inventa uma resposta confiante. Ele devolve question estruturada e diagnóstico NL_TARGET_AMBIGUO.
...
- src/config/agentic_assembly/models.py
  ...
  - Comportamento confirmado: a API expõe auto, workflow, agent_supervisor e deepagent_supervisor.
