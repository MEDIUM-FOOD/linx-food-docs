...
Esta etapa enquadra o briefing em um alvo agentic concreto. Ela existe porque workflow, agent_supervisor e deepagent_supervisor nao compartilham o mesmo contrato estrutural. Sem esse enquadramento, o sistema geraria AST em trilho errado e a validacao posterior viraria remendo.
...
2. se o target for auto e houver prompt, o IntentParser classifica o briefing e escolhe workflow, agent_supervisor ou deepagent_supervisor;
3. se nao houver prompt suficiente, o service tenta inferir o alvo pela estrutura do YAML base;
4. se multi_agents existir e algum execution.type for deepagent, o alvo vira deepagent_supervisor;
5. se multi_agents existir sem deepagent, o alvo vira agent_supervisor;
...
4. o resolved_target vira deepagent_supervisor;
5. todo o restante da esteira usa esse alvo para preflight, draft, validate e confirm.

O valor desta etapa e impedir que a plataforma trate uma intencao de supervisor ou deepagent como se fosse apenas um workflow genérico.
