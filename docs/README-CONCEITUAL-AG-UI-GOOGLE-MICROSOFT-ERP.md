...
Esse ponto e importante para a estrategia de produto. O repositorio nao esta apenas com uma demo AG-UI. Ele ja tem um boundary executavel, um registry de adapters, um discovery publico, um runtime compartilhado, replay e um caminho explicito para agent, deepagent e workflow. Isso significa que a plataforma ja possui o esqueleto de uma camada agentic de interface reaproveitavel.
...
HIL significa human in the loop, ou seja, uma decisao humana inserida no fluxo. No slice atual, agent e deepagent podem encerrar o run com outcome de interrupcao e depois continuar pelo mesmo endpoint AG-UI. Workflow, por sua vez, ainda nao tem resume AG-UI suportado neste boundary.
...
3. Execucao streaming em /ag-ui/runs.
4. Replay sanitizado por run e por thread.
5. Adapters registrados para agent, deepagent, workflow e retail_demo.
6. Runtime compartilhado do frontend em JavaScript puro.
7. Materializacao governada de dashboard dinamico.

Isso muda o enquadramento correto da feature. O AG-UI deste repositorio nao e apenas a demo de varejo. A demo de varejo e a vitrine mais tangivel do slice. Mas a infraestrutura ja serve tambem como boundary agentic mais amplo, com executionKind para agent, deepagent e workflow.
...
1. A pagina monta um payload AG-UI com threadId, runId, executionKind, input, metadata e uma fonte explicita de configuracao.
...
```mermaid
flowchart LR
    A[Tela corporativa] --> B[POST /ag-ui/runs]
    B --> C[AgUiRunOrchestrator]
    C --> D{executionKind}
    D --> E[agent]
    D --> F[deepagent]
    D --> G[workflow]
    D --> H[retail_demo]
    H --> I[dyn_sql governado]
    H --> J[DashboardSpec segura]
    E --> K[SSE de eventos AG-UI]
    F --> K
    G --> K
    I --> K
    J --> K
    K --> L[Runtime JS compartilhado]
    L --> M[Sidecar]
    L --> N[Tela principal]
```
...
### 10.3. Orquestracao do lifecycle
...
### 10.6. Runtime compartilhado do frontend
...
Uma nova pagina pode mandar request para /ag-ui/runs, consumir SSE e reconstruir estado via runtime compartilhado ou implementacao propria compatível.

### 12.2. Reaproveitando o pacote interno de runtime

O pacote interno @prometeu/ag-ui-runtime reexporta cliente, store, sidecar, renderers, timeline e validadores de dashboard. Ele nao e publicado como pacote externo publico, mas ja funciona como fachada interna de reuso.
...
Como o AG-UI ajuda: os runtimes agent e deepagent ja conseguem encerrar o run em interrupt e retomar pelo mesmo boundary AG-UI.
...
3. executionKind inexistente ou capability nao autorizada.
...
1. O replay atual e em memoria de processo. Ele nao e persistencia duravel cross-restart.
2. Workflow ainda nao suporta resume AG-UI neste boundary.
...
- Entendi que agent, deepagent e workflow usam o mesmo boundary, com limites diferentes.
...
- src/api/services/ag_ui_adapter_registry.py
  ...
  - Comportamento confirmado: registry explicito com agent, deepagent, workflow e retail_demo.
...
- src/api/services/ag_ui_event_store.py
  ...
  - Simbolo relevante: InMemoryAgUiEventStore, sanitize_ag_ui_event_payload.
  - Comportamento confirmado: replay por run e thread com redacao de segredos.
